        ...
        VStack(spacing: 8) {
            ...
            HStack(spacing: 12) {
                ...
                .background(GeometryReader { geo in
                    Color.clear
                        .onAppear { engine.speedButtonFrame = geo.frame(in: .global) }
                        .onChange(of: geo.frame(in: .global)) { engine.speedButtonFrame = $0 }
                })
            }
    ...
    var body: some View {
        if engine.showSpeedMenu {
            GeometryReader { menuGeo in
                let origin = menuGeo.frame(in: .global).origin
                let btn = engine.speedButtonFrame
                // Menu left edge at button right edge,
                // vertically centered so highlighted item aligns with button
                let localX = btn.maxX - origin.x + 28
                let localY = btn.midY - origin.y

                ZStack {
                    // Dismiss layer — taps outside menu close it
                                ...
                                VStack(spacing: 0) {
                                    ...
                                    ForEach(VinylEngine.speedOptions.reversed(), id: \.self) { speed in
                                        GeometryReader { geo in
                                            ...
                                            .preference(key: SpeedMenuPositionsPreferenceKey.self,
                                                        value: [speed: geo.frame(in: .named("speedMenu")).midY])
                                        }
                                        .frame(height: 28)
                                        .id(speed)
                                    }
